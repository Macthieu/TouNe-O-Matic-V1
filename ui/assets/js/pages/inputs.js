...
export async function render(root){
  ...
  const useBackend = AppConfig.transport === "rest";
  const defaults = {
    analog: [
  ...
  async function fetchAnalogState(){
    if(!useBackend) return null;
    try {
      return await apiJson("/analog/state");
    } catch {
      return null;
    }
  }

  async function pushRouteToBackend(inputId, outputId, enabled){
    if(!useBackend) return;
    try {
      analogState = await apiJson("/analog/route", {
        method: "POST",
      ...
      analogMode = analogState?.mode || analogMode;
    } catch {
      // ignore backend errors here; local state remains usable
    }
  }

  async function pushRoutesToBackend(nextRoutes, replace = false){
    if(!useBackend) return;
    try {
      analogState = await apiJson("/analog/routes", {
        method: "POST",
        headers: {"Content-Type": "application/json"},
        body: JSON.stringify({
          routes: nextRoutes,
  ...
  async function setAnalogMode(mode){
    if(mode !== "pure" && mode !== "cast") return;
    analogMode = mode;
    if(useBackend){
      try {
        analogState = await apiJson("/analog/mode", {
          method: "POST",
          headers: {"Content-Type": "application/json"},
          body: JSON.stringify({mode}),
        });
        analogMode = analogState?.mode || mode;
      } catch {
        // ignore backend errors; keep UI responsive
      }
    }
    renderPage();
  }
  ...
  function toggleRoute(inputId, outputId){
    ...
    const enabled = !routes[key];
    if(enabled) routes[key] = true;
    else delete routes[key];
    saveRoutes();
    void pushRouteToBackend(inputId, outputId, enabled);
  }
    ...
    reader.onload = ()=>{
      try {
        ...
        if(payload?.routes && typeof payload.routes === "object"){
          ...
          saveRoutes();
          void pushRoutesToBackend(routes, true);
        }
        renderPage();
      } catch {
        // ignore invalid file
  ...
  function renderPage(){
    ...
    if(useBackend){
      actions.push(
        button(analogMode === "pure" ? "Mode pur actif" : "Mode pur", {
          onClick: ()=>{ void setAnalogMode("pure"); }
        }),
        button(analogMode === "cast" ? "Mode cast actif" : "Mode cast", {
          onClick: ()=>{ void setAnalogMode("cast"); }
        }),
      );
    }
    ...
    summary.body.innerHTML = `
      ...
      <div class="muted small" style="margin-top:8px;">
        Mode analogique: <b>${analogMode === "cast" ? "cast (ADC -> Snapcast)" : "pur (commutation analogique)"}</b>
      </div>
    `;
    if(editMode){
  ...
  if(useBackend){
    analogState = await fetchAnalogState();
    if(analogState && typeof analogState === "object"){
      analogMode = analogState.mode === "cast" ? "cast" : "pure";
      if(analogState.routes && typeof analogState.routes === "object"){
        routes = analogState.routes;
        saveRoutes();
      }
    }
  }
